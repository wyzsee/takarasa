...
export default function DetailWorkshopPage() {
    const navigate = useNavigate();
    const { id } = useParams(); // 1. Ambil ID dari URL

    // 2. State untuk menampung detail acara (workshop), loading, dan error
    ...
    // 4. useEffect untuk mengambil data detail dari API
    useEffect(() => {
        if (!id) return;
        ...
        async function fetchDetailAcara() {
            setIsLoading(true);
            try {
                // Panggil endpoint /acara/{id}
                const response = await api.get(`/acara/${id}`);
                setAcaraDetail(response.data);
                setError(null);
            } catch (err) {
                console.error("Gagal mengambil detail data workshop:", err);
                setError("Data workshop tidak ditemukan.");
            } finally {
                setIsLoading(false);
            }
        }
            ...
            <main className="flex-grow w-full px-6 pb-4 overflow-y-auto">
                ...
                <div className="flex flex-col items-start gap-4 p-4 backdrop-blur-sm rounded-2xl">
                    <img
                        ...
                        src={`http://localhost:8000${acaraDetail.image_path}`}
                    />
